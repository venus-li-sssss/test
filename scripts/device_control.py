...
# --------------------------------------------------------------------------- #
# 滑动条控制（set_slider）
#   用于设置 APP 内的滑动条（如音效音量、亮度等）到指定值。
#   支持按文字定位滑动条，自动读取当前值，通过拖动或点击设置目标值。
#   已处理滑动条吸附行为：若点击不精确，自动微调。
# --------------------------------------------------------------------------- #
def _parse_bounds(bounds):
    """解析 bounds，支持 dict 或字符串格式。
...
def _find_slider_bounds(d, anchor_xpath, max_scroll=3):
    """查找滑动条的边界坐标。
    返回 (left_x, right_x, center_y, current_value, max_value) 或 None。
    """
    # 先尝试直接找滑动条元素（SeekBar）
    for _ in range(max_scroll + 1):
        # 方法1：通过 anchor 的兄弟/子元素找 SeekBar
        if anchor_xpath:
            # 查找 anchor 同级的 SeekBar
            seekbar_xpath = f"{anchor_xpath}/following-sibling::*//android.view.View[@resource-id='com.ninebot.segway:id/seekBar']"
            ele = d.xpath(seekbar_xpath)
            if ele.exists:
                info = ele.info
                ...
                if parsed:
                    ...
                    # 尝试读取当前值（找旁边的 TextView）
                    current_val = _read_slider_value(d, anchor_xpath)
                    return left_x, right_x, center_y, current_val, 100  # 默认最大值 100

        # 方法2：全局搜索 SeekBar
        for e in d(className="android.view.View", resourceId="com.ninebot.segway:id/seekBar"):
            info = e.info
            bounds = info.get("bounds")
            parsed = _parse_bounds(bounds)
            if parsed:
                left_x, top_y, right_x, bottom_y = parsed
                center_y = (top_y + bottom_y) // 2
                current_val = _read_slider_value_nearby(d, left_x, top_y, right_x, bottom_y)
                return left_x, right_x, center_y, current_val, 100
...
def _read_slider_value(d, anchor_xpath):
    """读取滑动条旁边的当前值文本（如 '83%' 中的 83）。"""
    # 方法1：通过 xpath 找 anchor 后面的百分比文本
    val_xpath = f"{anchor_xpath}/following-sibling::*//android.widget.TextView"
    for e in d.xpath(val_xpath).all():
        text = e.info.get("text", "")
        import re
        if re.search(r'\d+%', text):  # 匹配 "83%" 格式
            m = re.search(r'(\d+)', text)
            if m:
                return int(m.group(1))

    # 方法2：通过 anchor 的兄弟节点找包含 % 的文本
    if anchor_xpath:
        sibling_xpath = f"{anchor_xpath}/following-sibling::android.widget.TextView"
        for e in d.xpath(sibling_xpath).all():
            text = e.info.get("text", "")
            import re
...
def _read_slider_value_nearby(d, left_x, top_y, right_x, bottom_y):
    """通过坐标附近搜索读取滑动条当前值。"""
    import re
    # 在滑动条上方找百分比文本（如 "83%"）
    search_top = max(0, top_y - 150)
    search_bottom = top_y + 20
    for e in d(className="android.widget.TextView"):
        info = e.info
        text = info.get("text", "")
        # 只处理包含 % 的文本（如 "83%"）
        ...
        parsed = _parse_bounds(bounds)
        if parsed:
            ex1, ey1, ex2, ey2 = parsed
            # 检查是否在滑动条上方附近
            if search_top <= ey1 <= search_bottom and left_x - 100 <= ex1 <= right_x + 100:
...
def _find_slider_thumb(d, anchor_xpath, max_scroll=3):
    """查找滑动条的滑块（thumb）位置和滑动条边界。
    返回 (thumb_x, thumb_y, left_x, right_x, center_y) 或 None。
    """
    for _ in range(max_scroll + 1):
        # 方法 1：通过 anchor 找 SeekBar
        if anchor_xpath:
            seekbar_xpath = f"{anchor_xpath}/following-sibling::*//android.view.View[@resource-id='com.ninebot.segway:id/seekBar']"
            ele = d.xpath(seekbar_xpath)
            if ele.exists:
                ...
                if parsed:
                    ...
                    if current_val is not None:
                        ...
                        thumb_x = left_x + int(width * current_val / 100)
                        return thumb_x, center_y, left_x, right_x, center_y
...
def do_set_slider(d, opts):
    """设置滑动条到指定值（零误差版：按住 - 滑动 - 微调 - 松开）。
    
    核心逻辑：
      1. 按住滑块（touch.down）
      2. 滑动到目标附近（touch.move）
      3. 实时读取值，微调位置（touch.move）
      4. 精确匹配后松开（touch.up）
    ...
    参数：
      --text/--desc/--id/--xpath  定位滑动条（或其标签文字）
      --value                     目标值（必填，0~max-value）
      --max-value                 最大值（默认 100）
      --step-size                 大步长（默认 15，快速接近目标）
      --fine-step                 微调步长（默认 2，精确匹配）
      --read-delay                每次移动后等待读取值的秒数（默认 0.3）
      --max-overshoot             最大过冲次数（默认 3，防止无限循环）
    """
    anchor = _build_anchor_xpath(opts)
    if not anchor:
        return {"ok": False, "message": "no selector (text/desc/id/xpath)"}

    target_value = int(opts.get("value") or 0)
    max_value = int(opts.get("max_value") or 100)
    step_size = int(opts.get("step_size") or 15)
    fine_step = int(opts.get("fine_step") or 2)
    read_delay = float(opts.get("read_delay") or 0.3)
    max_overshoot = int(opts.get("max_overshoot") or 3)

    # 查找滑动条边界
    slider_info = _find_slider_thumb(d, anchor)
    if not slider_info:
        return {"ok": False, "message": "slider not found", "target": anchor}

    thumb_x, thumb_y, left_x, right_x, center_y = slider_info
    width = right_x - left_x

    # 读取当前值
    current_value = _read_slider_value(d, anchor)
    if current_value is None:
        current_value = _read_slider_value_nearby(d, left_x, center_y - 100, right_x, center_y + 100)

    if current_value is None:
        return {"ok": False, "message": "无法读取当前值", "target": anchor}

    # 用当前值重新计算 thumb 位置（更准确）
    thumb_x = left_x + int(width * current_value / max_value)

    # 计算目标 x 坐标
    target_x = left_x + int(width * target_value / max_value)
    ...
    def _read_current_value():
        """读取当前滑动条值"""
        val = _read_slider_value(d, anchor)
        if val is None:
            val = _read_slider_value_nearby(d, left_x, center_y - 100, right_x, center_y + 100)
        return val

    # ========== 按住滑块 ==========
    d.touch.down(thumb_x, center_y)
    time.sleep(0.3)
    history.append({"action": "touch_down", "x": thumb_x, "y": center_y})

    current_x = thumb_x
    overshoot_count = 0
    last_direction = 0  # 0=未移动，1=向右，-1=向左
    current_step_size = step_size

    # ========== 滑动 + 微调（不松手）==========
    step = 0
    while abs(current_value - target_value) > 0:
        step += 1
        if step > 100:  # 防止无限循环
            history.append({"action": "max_steps_reached", "current_value": current_value})
            break

        # 确定移动方向
        if current_value < target_value:
            direction = 1  # 需要增大值，向右拖
        else:
            direction = -1  # 需要减小值，向左拖
        ...
        if last_direction != 0 and direction != last_direction:
            ...
            if overshoot_count >= max_overshoot:
                ...
                break
            # 切换到微调模式
            current_step_size = fine_step

        last_direction = direction

        # 计算下一步位置
        next_x = current_x + direction * current_step_size
        next_x = max(left_x, min(right_x, next_x))

        # 移动（不松手）
        d.touch.move(next_x, center_y)
        time.sleep(read_delay)

        # 读取新值（等待"正在设置..."消失，最多等 2 秒）
        new_value = None
        for wait_attempt in range(8):
            new_value = _read_current_value()
        ...
        if new_value is None:
            ...
            history.append({"step": step, "action": "read_timeout_use_last", "x": next_x, "value": new_value})
        else:
            history.append({
                "phase": "coarse" if current_step_size > fine_step else "fine",
        ...
        # 如果值不再变化（卡住了），停止
        if len(history) >= 2:
            last_val = history[-1].get("value_after")
            prev_val = history[-2].get("value_after")
            if last_val is not None and prev_val is not None and last_val == prev_val:
                if len(history) >= 5:
                    break

    # ========== 阶段 2：微调（如果还没精确匹配）==========
    if not final_ok and abs(current_value - target_value) <= 3:
        # 在目标值附近 3 以内，用超小步长微调（仍然不松手）
        micro_step = 1
        for _ in range(20):  # 最多 20 次微调
            if current_value == target_value:
                final_ok = True
                break
            ...
            next_x = current_x + direction * micro_step
            next_x = max(left_x, min(right_x, next_x))
    ...
    # ========== 松开滑块 ==========
    d.touch.up(current_x, center_y)
    time.sleep(0.5)
    history.append({"action": "touch_up", "x": current_x, "y": center_y})

    # 最终读取值
    final_value = _read_current_value()
...
def build_parser():
    ...
    sp = sub.add_parser("set_slider", help="设置滑动条到指定值（零误差版：过冲回退算法）")
    sp.add_argument("--text", help="按文字定位滑动条（或其标签）")
    sp.add_argument("--desc", help="按 content-desc 定位")
    sp.add_argument("--id", help="按 resourceId 定位")
    sp.add_argument("--xpath", help="按 xpath 定位")
    sp.add_argument("--value", type=int, required=True, help="目标值（如 80 表示 80%）")
    sp.add_argument("--max-value", type=int, default=100, help="最大值（默认 100）")
    sp.add_argument("--step-size", type=int, default=15, help="大步长（默认 15，快速接近目标）")
    sp.add_argument("--fine-step", type=int, default=2, help="微调步长（默认 2，精确匹配）")
    sp.add_argument("--read-delay", type=float, default=0.3, help="每次移动后等待读取值的秒数（默认 0.3）")
    sp.add_argument("--max-overshoot", type=int, default=3, help="最大过冲次数（默认 3）")
...
def opts_from_args(args):
    ...
    keys = ["package", "text", "desc", "id", "xpath", "up", "out",
            ...
            "value", "max_value", "step_size", "fine_step", "read_delay", "max_overshoot"]
