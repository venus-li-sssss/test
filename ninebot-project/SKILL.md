...
> **未执行用例一律不填数据（v1.9.6 更新）**：`Blocked-NoRun` / `Blocked-NonSupport` 等**没有真正执行**的用例，在 Excel 中**J/K/L/M/P/Q/R 全部留空**，不得填写 verdict、Blocked 原因、实测记录，也不得嵌入任何截图。只有**确实执行过**的用例（PASS / FAIL）才填写 J  verdict，并在 K/L/M/P/Q/R 回填记录、备注与执行过程截图。这样能在 Excel 里一眼区分「有数据=执行过」与「全空白=未执行」，避免把佐证图或说明文字误当成某用例的执行证据。**教训**：早期 `fill_precise_range.py` 对所有用例统一填了 3 张图，还在 K 列写原因，导致 Blocked 行与 PASS/FAIL 行混淆——已全表修正。
