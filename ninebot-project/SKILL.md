...
8. **4G Only 测试集混入“4G 连接但期望不可控”用例**：如精准续航-01-010（4G 下开启期望“无法同步到车辆仪表”）、彩蛋功能-01-009（4G 下开启期望“提示连接蓝牙”）。当前大部分设置项已可通过 4G 云通道控制，此类用例已失效。→ 清理原则：**测试集名含 “4G Only” 时，只保留 4G 通道可执行的用例；凡前置写“4G 连接”但期望写“不能同步 / 提示连蓝牙 / 无法到仪表”的行，直接删除**。
...
### 8.0 ⭐ 通用测试执行工作流（最简原则，所有用例执行都走这套）

**不要为每条用例写新脚本、也不要在首页瞎点乱走。** 统一用下面 4 步，越简单越好；全程用 XML 文字（`texts`/`dump`）当眼睛、用截图当证据：

1. **读取页面导航**：当前页先 `texts`（或 `dump`）看有哪些可点入口/控件，用 XML 文字判断「能去哪、要点的字叫什么」。**不靠猜、不靠记忆、不乱点**。
2. **页面导航**：到目标页用页面树导航 `go_to_page --page <id>`（BFS 最稳，见 §8.10）；需逐级下钻用 `cmd --path "更多功能,xxx"`。**不要自己手写坐标点按**。
3. **页面滚动识别元素**：目标页上 `swipe` 滚动 + `texts`/`dump` 收集文字，定位要操作或读取的元素（长列表/折叠区尤其要滚到底）。
4. **执行操作 / 获取数据**：点按控件（`cmd`/`tap`/`retry`）或读元素属性与文字（数据获取）。判定一律以**元素属性（`checked`/`text`）** 为准。

> **截图是测试证据（平台要靠它证明你做了测试）**：每到达一个页面、每做一次关键操作，都必须 `screenshot` 存图；回填 Excel 时贴到 P/Q/R 列（按 §8.2.5 尺寸锁行内）。**没截图 ≈ 没做测试**。纯读取类页面也要截图存档。

> **简单优先**：能一条 helper 完成就一条（`cmd`/`tap`/`screenshot`/`go_to_page`/`texts`）；多步才用 `run --json` 串；脚本没有的兜底走 §8.13。**不要在脚本里为每个按钮新写 Python**。
...
> **4G Only 测试集清理铁律（v1.9.5 新增）**：测试集文件名含 `4G_Only` 时，只保留**可通过 4G 云通道执行/验证**的用例。凡前置条件写“4G 连接”但预期结果写“无法同步到车辆仪表 / 提示连接蓝牙 / 无法连接蓝牙 / 不同步”等“4G 不可控”假设的行，**直接删除**。已删除实例：精准续航-01-010、彩蛋功能-01-009。后续新增用例前先判断：该功能在 4G 连接下是否真的无法控制；若现在已能控制，则不应出现在 4G Only 测试集中。
...
- 真正需要 BLE 的只有：感应解锁、仪表盘显示剩余里程调节（仪表需蓝牙）、基本数据检查（车辆基础数据需蓝牙读取）、用车人管理卡片（需蓝牙/账号）、骑行记录卡片（记录存本地/需蓝牙）、NFC 钥匙、`ble_upgrade_app`（APP 侧固件刷写）。整车_4G_Only 测试集各模块「是否需 BLE」的完整判定表见 §8.5.5。
...
测试集 `IK-QP-012-07_ODM_QDM522_TestCase_整车_V4_4G_Only` 的「车辆详情」sheet 共 198 条用例。

> **核心认知（纠正 2026-08-09 误判）**：这是 **4G Only** 测试集，名字就说明——**绝大多数操作走 4G 云通道、在 APP 侧就能执行**，不要误判成"要物理按键 / 要蓝牙"而整批留空。开关机、闪灯鸣笛、车辆4G连接状态、车辆图片、更多功能卡片都可直接在 APP 执行 + 截图取证。只有明确需要「蓝牙直连 / 物理按键动作 / 实际骑行 GPS / 接充电器 / 整车无此硬件」的才留空或标 N/A。

**各模块判定表（执行前先看这张表）：**

| 模块 | 条数 | 判定 | 导航 / 执行方式 |
|---|---|---|---|
| 开关机检查 | 22 | ✅ 4G 可执行 | **首页**（电源滑块/滑动开机，回顶可见）；点 SOC 进电源页查看电量。`device_control.py power_on`/`power_off`（§8.11）。验证 APP 状态切换 + 必要时 `commands --watch` 链路核验 |
| 闪灯鸣笛检查 | 3 | ✅ 4G 可执行 | **在主页面**（控件名"闪灯鸣笛"）。`cmd --target "闪灯鸣笛" --action tap --start home` |
| 车辆4G连接状态 | 3 | ✅ 4G 可执行 | 读首页 / 设置页 4G 状态文字 |
| 车辆图片 | 7 | ✅ 4G 可执行 | 设备信息页（`go_to_page device_info`）读 车型/车架号/总里程；车辆图片本身在主页查看 |
| 更多功能卡片 | 2 | ✅ 4G 可执行 | 首页→更多功能（`go_to_page more_functions`） |
| 座筒开关检查 | 10 | ⛔ 标记 N/A | **该整车无座桶**，此功能不适用。**J 列填 `N/A`**、备注"该整车无座桶，此功能不适用"，整行其余留空，**不标 PASS/FAIL、不标 Blocked** |
| 基本数据检查 | 51 | ⏭️ BLE 跳过 | 需蓝牙读车辆基础数据 → 留空 |
| 感应解锁 | 19+1 | ⏭️ BLE 跳过 | 蓝牙近场感应 → 留空 |
| 用车人管理卡片 | 23 | ⏭️ BLE 跳过 | 需蓝牙 / 账号 → 留空 |
| 仪表盘显示剩余里程调节 | 7 | ⏭️ BLE 跳过 | 仪表需蓝牙 → 留空 |
| 骑行记录卡片 | 5 | ⏭️ BLE 跳过 | 记录需 BLE / 本地 → 留空 |
| 充电 | 27 | ⏭️ 物理跳过 | 需物理接充电器 → 留空 |
| 车辆定位卡片 | 12 | ⏭️ GPS 跳过 | 需实际 GPS 骑行 → 留空 |
| 车辆定位 | 3 | ⏭️ GPS 跳过 | 同上 → 留空 |
| 功能卡片 | 2 | 含 感应解锁(BLE)→留空 | 同感应解锁 |

**⭐ 可观测性边界（云指令类用例的判定依据，2026-08-10 用户明确）**
走 4G 云通道的指令类功能（开关机、闪灯鸣笛等），完整链路为：

```
APP 点击 → 平台下发指令 → 平台接收设备返回 → 整车执行物理动作
```

- **APP 侧可见**：点击后的下发提示 / 状态变化（toast、按钮态、状态文字）。`device_control.py` 的 `tap`/`cmd` + `screenshot` 取证。
- **平台侧可见**：`commands --watch`（§9）能看到该指令的下发记录与设备 `resp_code`/`status` 回应。
- **整车物理动作不可见**：闪灯、鸣笛、实际开关机等物理效果在测试环境**无法直接观察**。**绝不能**因为"没看到整车动作"就判 FAIL 或留空。
- **判定 PASS 的依据**：APP 侧出现正常下发反馈 **且** 平台侧有该指令的下发与成功回应（`resp_code=01`/`status=3`）。物理动作不可见属正常，不作为判据，也不要脑补"应该有倒计时/进度条"之类不存在的 UI。
- **范例（闪灯鸣笛）**：APP 点击「闪灯鸣笛」→ 平台下发指令 → 设备返回成功即判 PASS；整车是否真响铃闪灯不在可观测范围内，截图证明 APP 已下发 + 平台有成功回应即可。

**判定优先级（执行任意用例前按此判断）：**
1. 看用例「前置/步骤」是否含"APP 点击 / 4G 连接 / 查看主页" → 基本都 **✅ 可执行**，直接导航 + 操作 + 截图。
2. 只有明确需「蓝牙直连（仪表/基本数据/用车人管理/感应解锁）」「物理接充电器（充电）」「实际骑行 GPS（车辆定位）」或「整车无此硬件（座筒→N/A）」才留空 / 标 N/A。
3. 拿不准就**直接问用户**，不要默认"不能执行"。

### 8.5.6 ⭐ APP 页面结构与功能位置标准（2026-08-10 用户纠正：以页面为主导）

找元素顺序：**先定页面 → 再看该页面内 顶层/中层/底层 → 按滑动策略定位 → 按操作描述执行**。位置与数据以九号 APP **实机逐页遍历（2026-08-10，脚本 traverse_pages.py / scan_more.py 抓屏+dump）**为准，**不能靠记忆或口头纠正补全**——新增/变化的功能必须连设备重新遍历。

**滑动策略铁律**：顶层=滑动到顶部即可见；底层=滑动到页面底部；中层=先滑动到顶部再逐渐往下滑寻找。**⚠️ 不能从底部往上滑**——已在底部时往上滑永远定位不到目标（2026-08-09 实测教训）。

**⭐ 兜底搜索（不同机型/APP 版本功能层级会偏移，2026-08-10 用户纠正）**：某功能按本表标注的「标准层」找不到时，**不要直接断定没有 / 留空跳过**，而应先把整页从顶到底完整滑一遍逐屏核对——尤其是**中间层**（很多功能在部分机型上会从顶层/底层偏移进中层）。只有「回顶 → 逐屏下滑 → 滑到底」全流程都无该文字，才能确认该功能不在本页 / 需对应环境（BLE/物理/GPS）。

**① 页面清单与跳转**
| 页面 | 入口 / 进入方式 | 返回方式 |
|---|---|---|
| 首页 home（MainOversea） | 启动 APP | — |
| 更多功能 more_functions | 首页点「更多功能」 | back 回首页 |
| 设备信息 device_info | 首页 / 更多功能 点「设备信息」 | back |
| 电源页 | 首页点 SOC（电量 %） | back |

**② 各页面内分层（顶层 / 中层 / 底层）+ 操作描述**

**▸ 首页 home**（实机值：电量 84%/84.0、续航 99.8 km、总里程 28.3 km、最近骑行 0.2 km、定位「三山科创中心B区」、拥有爱车第110天、车型 赛格威越野电摩Xaber 300美洲版）
| 层级 | 功能 / 数据 | 操作描述 | 说明（实机值） |
|---|---|---|---|
| 顶层 | 电量 / SOC（顶部大数字） | 查看；**点击该数字区域**进入电源页 | 84% / 84.0 |
| 顶层 | 续航 | 直接查看 | 99.8 km |
| 顶层 | 开关机（电源滑块 / 滑动开机） | 在电源滑块处**滑动**以开机；开机后电源页点「关机」 | ✅ 4G；当前为「滑动开机」关机态 |
| 顶层 | 闪灯鸣笛 | 点击→下发 4G 指令 | ✅ 4G；平台返回成功即 PASS（整车响铃闪灯不可见、无倒计时） |
| 顶层 | 定位卡片（大概位置） | 直接查看 | 三山科创中心B区（仅大概位置，非精确 GPS） |
| 顶层 | 总里程 | 直接查看 | 28.3 km |
| 顶层 | 最近骑行 | 直接查看 | 0.2 km |
| 顶层 | 车型 | 直接查看 | 赛格威越野电摩Xaber 300美洲版 |
| 顶层 | 仪表盘（剩余里程等） | （需蓝牙，暂不操作） | ⏭️ BLE 跳过 |
| 顶层 | 感应解锁 | （需蓝牙，暂不操作） | ⏭️ BLE 跳过 |
| 顶层 | 未绑定电池提示 | 直接查看 | 「发现未绑定电池 / 绑定后可提供更好的电池服务 / 请先连接蓝牙」（BLE 相关） |
| 顶层 | 新手任务 / 了解车辆 / 新手教学 | 点击进入 | 引导类 |
| 顶层 | 服务与帮助 | 点击进入 | 「点击查看设备服务与帮助功能」 |
| 顶层 | 拥有爱车第 N 天 | 直接查看 | 实机：第110天 |
| 顶层 | 更多功能（入口） | 点击「更多功能」 | 跳转 |

**▸ 更多功能 more_functions**（实机扫描 2026-08-10，**共 25 项**，按首次出现屏次分层；部分项带 APP 内说明文字）
| 层级 | 功能 / 数据 | 操作描述 | APP 内说明（实机抓到） |
|---|---|---|---|
| 顶层（滑到顶部） | 安防设置 | 点击进入 | — |
| 顶层 | 灯光设置 | 点击进入 | — |
| 顶层 | 音效设置 | 点击进入 | — |
| 顶层 | NFC 和密码设置 | 点击进入 | — |
| 顶层 | 快捷功能定义 | 点击进入 | — |
| 顶层 | 驻车感应 | 开关切换 / 点击进入 | 开启后，车辆可感应监测边撑是否收起。关闭后仍可正常骑行。 |
| 顶层 | 自动锁车设置 | 开关 / 点击进入 | — |
| 顶层 | 低电量延长续航 | 开关切换 | 开启后，当电量低于20%时会自动调整车速上限以提升续航里程。 |
| 顶层 | 电子刹车 | 开关切换 | 开启后，机械刹车时会切断动力并伴随电子刹车。关闭后只依赖机械刹车。 |
| 中层（先回顶再下滑） | 自动驻车 | 开关切换 | 开启时，停车后捏住刹车3秒即可自动驻车。 |
| 中层 | 速度安全锁 | 开关 / 设置 | 当前「关」 |
| 中层 | 精准续航 | 开关 / 了解 | 根据您的骑行习惯计算剩余里程，里程预估更准确（附「了解精准续航」） |
| 中层 | 牵引力控制系统（TCS） | 开关切换 | 关闭后，湿滑路面下大动力输出可能导致打滑等危险。 |
| 中层 | 氮气模式 | 开关切换 | 开启后，在高挡位下快速拧动油门会触发氮气模式。 |
| 中层 | 彩蛋功能 | 开关切换 | 开启后，某些特殊日期可能会有开关机彩蛋音效。*彩屏仪表车型可能会有惊喜彩蛋皮肤。 |
| 中层 | 能量回收强度 | 设置 | 当前「关闭」 |
| 底层（滑到页面底部） | 骑行模式设置 | 点击进入 | 可根据个人习惯自定义档位模式。 |
| 底层 | 公英制切换 | 切换 | 当前「公制」 |
| 底层 | 转把设置 | 点击进入 | 支持油门与转把相关设置 |
| 底层 | 倒车断电 | 开关切换 | 车辆侧翻时自动切断动力；进行飞跳等专业动作时请关闭此功能 |
| 底层 | 安心守护 | 点击进入 | 设置安全区域，车辆驶出后收到提醒 |
| 底层 | 实验室 | 点击进入 | 实验功能 |
| 底层 | 电池信息与设置 | 点击进入 | 跳转电池页 |
| 底层 | 设备信息 | 点击进入 | 跳转 device_info 页 |
| 底层 | 解绑车辆 | 点击进入 | ⚠️ 危险操作，一般测试不执行 |

**▸ 设备信息 device_info**（实机扫描 2026-08-10）
| 层级 | 功能 / 数据 | 操作描述 | 说明 |
|---|---|---|---|
| 顶层 | 车辆型号 / 激活日期 | 直接查看 | 如 赛格威越野电摩Xaber 300美洲版 / 2026.01.21 激活 |
| 顶层 | 车架号 | 直接查看 | LTUY26WL7T1000114 |
| 顶层 | 设备序列号 | 直接查看 | 48DGZ2602J0022 |
| 顶层 | 车辆总里程 | 直接查看 | 28.3KM |
| 顶层 | 固件详情（标题） | 直接查看 | 各模块版本汇总入口 |
| 中层（回顶后下滑） | 版本号（仪表控制器 / 中控 / 彩屏仪表 / 电池 / 电机控制器 / 充电器） | 直接查看 | 仪表控制器 v1.1.5、中控 v2.3.14、彩屏仪表 v3.1.7、电池 v2.0.0.8、〈电机控制器 版本未捕获，待复核〉、充电器 v5.0.13 |
| 中层 | 固件更新日志 | 点击查看 | — |
| 中层 | 固件智能升级 | 开关切换 | 开启后，车辆会在您不使用的时段静默更新到最新稳定版固件（电机控制器等固件目前仅支持开机升级，因此仍需点击「检查固件更新」手动完成升级）。 |
| 中层 | 检查固件更新 | 点击执行 | 手动检查升级 |

**▸ 电源页 / 电池页（点首页 SOC 数字进入，activity=NBReactActivity）**（实机扫描 2026-08-10，含真实 SOC 数据）
| 层级 | 功能 / 数据 | 操作描述 | 说明（实机值） |
|---|---|---|---|
| 顶层 | 主电池电量 | 直接查看 | 84% |
| 顶层 | 剩余电量 / 电压 / 温度 | 直接查看 | 剩余电量 81.0 V、电压 33℃（⚠️ 单位实录如此，温度/电压字段同屏） |
| 顶层 | 应急通信电池 | 直接查看 | 100% |
| 顶层 | 充电上限调节 | 设置充电上限百分比 | 降低电池充电上限有助于提高电池的使用寿命。 |
| 顶层 | 电池管理 | 点击进入 | 可查看电池序列号，以及对电池进行绑定或解绑。 |
| 顶层 | 公共充电桩适配 | 开关 / 设置 | 配合官方高功率充电器使用，当公共充电桩因限制充电功率而无法充电时，可以尝试调整充电功率。 |
| 中层（回顶后下滑） | 电池详情 | 点击进入 | — |
| 中层 | 低电量智能提醒 | 开关 | 车辆电量过低时，智能提醒充电。 |
| 中层 | 电池保养指南 | 点击进入 | — |

> 蓝牙类（感应解锁 / 仪表盘）位置在**首页**，不要因跳过就不标位置；其余 BLE / 物理 / GPS 类（基本数据、用车人管理、充电、车辆定位、骑行记录）需相应环境，留空或标 N/A（座桶→N/A）。各页面具体功能是否纳入某条用例、是否 BLE/物理，按具体用例步骤判定；**本表只负责「在哪个页面、页面内第几层、怎么滑、怎么操作、真实显示什么数据」**。更多功能页功能以实机扫描为准，新增功能直接补进本表对应层级。
